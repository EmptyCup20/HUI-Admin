...
define(["markdown", "fileupload"], function (markdown) {
    var DocManage = Backbone.View.extend({
        ...
        render: function (id) {
            var that = this;
            this.id = id;
            $.get("/html/design/designManageEdit.html").done(function (data) {
                $(".page").html(data);
                that.setElement("#designManage");
        ...
        getDoc: function (id) {
            var that = this;
            $.ajax({
                url:window.App.apiIp +  "/admin/design/getDocById",
                data:{
                    id:id
                },
                error:function(err){
        ...
        submit: function () {
            ...
            $.ajax({
                url:window.App.apiIp +  "/admin/design/" + (this.id ? 'updateDoc':'addDoc'),
                method: "post",
                data: this.id ? $.extend(fromData,{id:this.id}) : fromData
            }).done(function (data) {
                that.id ? alert('修改成功') : alert('保存成功');
                window.location.href= '/#/designManage'
            })
        }
    });
    return DocManage;
